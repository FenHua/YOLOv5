...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.img_size = img_size                                     # 网络输入尺度
        self.augment = augment                                       # 是否采用增广
        self.hyp = hyp                                               # 超参
        self.image_weights = image_weights                           # 图片采样使用
        self.rect = False if image_weights else rect                 # 矩形训练
        self.mosaic = self.augment and not self.rect                 # 训练时将四张图片转一张
        self.mosaic_border = [-img_size // 2, -img_size // 2]        # mosaic（嵌入图片）边界
        self.stride = stride                                         # 模型下采样总步长
        self.path = path                                             #
        self.albumentations = Albumentations() if augment else None  # albu增强
        try:
            f = []  # 获取数据集路径
            for p in path if isinstance(path, list) else [path]:
                p = Path(p)     # 整理成与系统无关的路径，“/”
                if p.is_dir():  # dir
        ...
        # Check cache
        self.label_files = img2label_paths(self.img_files)     # 类别信息
        ...
        if self.rect:
            ...
            s = self.shapes         # 宽长
            ar = s[:, 1] / s[:, 0]  # 长宽比例
            irect = ar.argsort()    # 排序取索引
            self.img_files = [self.img_files[i] for i in irect]      # 图片文件
            self.label_files = [self.label_files[i] for i in irect]  # label文件
            self.labels = [self.labels[i] for i in irect]            # label
            self.shapes = s[irect]  # wh                             # 宽长排序
            ar = ar[irect]          # 比例排序
            # 设置训练图片的形状
            shapes = [[1, 1]] * nb
            for i in range(nb):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, labels = load_mosaic(self, index)   # 加载 mosaic
            shapes = None
            # MixUp增强
            if random.random() < hyp['mixup']:
                img, labels = mixup(img, labels, *load_mosaic(self, random.randint(0, self.n - 1)))
        else:
            img, (h0, w0), (h, w) = load_image(self, index)  # 加载图片
            # resize操作
        ...
        nl = len(labels)  # 标签的数量
        if nl:
            labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=img.shape[1], h=img.shape[0])  # xyxy to xywh normalized（标准化）
        if self.augment:
            # albu 增强
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        img, _, (h, w) = load_image(self, index)   # 加载图片
        if i == 0:  # top left
...
def flatten_recursive(path='../datasets/coco128'):
    ...
    # /** 匹配任意多级路径如/aa/bb/cc
    # /* 仅可以匹配一级路径如/aa  /? 仅可以匹配一级路径且最多只有一个字符如/a
...
def extract_boxes(path='../datasets/coco128'):
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:] in img_formats:
            im = cv2.imread(str(im_file))[..., ::-1]  # 取图片并将 BGR 转为 RGB
            h, w = im.shape[:2]                       # 长宽
            lb_file = Path(img2label_paths([str(im_file)])[0])   # 获取该图对应的label
...
def autosplit(path='../datasets/coco128/images', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    indices = random.choices([0, 1, 2], weights=weights, k=n)                       # 将每张图片随机划分为train/val/test
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False):
    ...
    if verbose:
        print(json.dumps(stats, indent=2, sort_keys=False))   # 输出
    return stats
